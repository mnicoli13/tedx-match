...
import json
import pyspark
from pyspark.sql.functions import col, collect_list, struct, split, explode
...
#--------------------------------------------------
tedx_dataset_path = "s3://tedx-2025-data-mp-19092025/final_list.csv"
details_dataset_path = "s3://tedx-2025-data-mp-19092025/details.csv"
tags_dataset_path = "s3://tedx-2025-data-mp-19092025/tags.csv"
related_videos_path = "s3://tedx-2025-data-mp-19092025/related_videos.csv"
images_path = "s3://tedx-2025-data-mp-19092025/images.csv"
users_dataset_path = "s3://tedx-2025-data-mp-19092025/users.csv"
...
users_exploded = users_dataset \
    .withColumn("like_id", explode(split(col("likes"), ",")))
    
user_likes_joined = users_exploded.join(
    tedx_dataset_agg,
    users_exploded.like_id == tedx_dataset_agg["_id"],
    "left"
)

user_likes_agg = user_likes_joined.groupBy(
    "user_id", "first_name", "last_name", "username", "thumbnail"
).agg(
    collect_list(
        struct(
            col("_id"),
...
users_dynamic_frame = DynamicFrame.fromDF(user_likes_agg, glueContext, "users")
...
glueContext.write_dynamic_frame.from_options(
    frame=users_dynamic_frame,
    connection_type="mongodb",
    connection_options=write_users_options
)
